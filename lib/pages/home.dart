...
//订阅刷新通知
  ...
  loadData() {
    HomeApi().requestSubscribeUrl(
        success: (data) {
          SubscriptionModel subscriptionModel = data;
          UserInfo.instance.subscriptionModel = subscriptionModel;
          ref.read(initialUrlProvider.notifier).state =
              subscriptionModel.subscribeUrl;
        },
        fail: (code, msg) {});
  }
  ...
  //获取公告内容
  getNotice() {
    HomeApi().requestNotice(
      success: (data) {
        if (data.isNotEmpty) {
          var noticeContent = data[0].content?.replaceAll('\n', '');
          UserInfo.instance.noticeList = data;
          UserInfo.instance.noticeContent = noticeContent ?? '';
          ref.read(noticeContentProvider.notifier).state = noticeContent;
        }
      },
      fail: (code, msg) {},
    );
